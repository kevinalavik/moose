...
$(IMAGE_NAME).iso: limine-binary/limine kernel ramfs.cpio
	rm -rf iso_root
	mkdir -p iso_root/boot
	mkdir -p iso_root/moose
	cp -v kernel/bin/kernel iso_root/moose/
	cp -v ramfs.cpio iso_root/moose/
	mkdir -p iso_root/boot/limine
	cp -v conf/limine.conf limine-binary/limine-bios.sys limine-binary/limine-bios-cd.bin limine-binary/limine-uefi-cd.bin iso_root/boot/limine/
...
$(IMAGE_NAME).hdd: limine-binary/limine kernel ramfs.cpio
	rm -f $(IMAGE_NAME).hdd
	dd if=/dev/zero bs=1M count=0 seek=64 of=$(IMAGE_NAME).hdd
	PATH=$$PATH:/usr/sbin:/sbin sgdisk $(IMAGE_NAME).hdd -n 1:2048 -t 1:ef00 -m 1
	./limine-binary/limine bios-install $(IMAGE_NAME).hdd
	mformat -i $(IMAGE_NAME).hdd@@1M
	mmd -i $(IMAGE_NAME).hdd@@1M ::/EFI ::/EFI/BOOT ::/boot ::/boot/limine ::/moose
	mcopy -i $(IMAGE_NAME).hdd@@1M kernel/bin/kernel ::/moose
	mcopy -i $(IMAGE_NAME).hdd@@1M ramfs.cpio ::/moose
	mcopy -i $(IMAGE_NAME).hdd@@1M conf/limine.conf limine-binary/limine-bios.sys ::/boot/limine
...
clean:
	$(MAKE) -C kernel clean
	rm -rf iso_root $(IMAGE_NAME).iso $(IMAGE_NAME).hdd ramfs.cpio
