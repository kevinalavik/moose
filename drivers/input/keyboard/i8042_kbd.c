#include <input/keyboard/i8042_kbd.h>
...
static void i8042_kbd_irq(int_frame_t *frame)
{
	(void)frame;
...
static int i8042_kbd_probe(device_t *dev)
{
	uint32_t gsi = 1;
	...
	if (ps2_keyboard_init() < 0) {
		klog("i8042_kbd", "no keyboard");
		return -ENODEV;
	}

	kbd_present = 1;
	uint8_t vector = IRQ_BASE + (uint8_t)gsi;
	irq_register(vector, i8042_kbd_irq);
	apic_gsi_set_mask(gsi, 0);
	return 0;
}

static const platform_device_id_t i8042_kbd_ids[] = {
	{ "PNP0303" },
	{ NULL },
};

static platform_driver_t i8042_kbd_driver = {
	.driver = {
		.name   = "i8042_kbd",
		.probe  = i8042_kbd_probe,
		.remove = NULL,
	},
	.id_table = i8042_kbd_ids,
};

int i8042_kbd_init(void)
{
	return platform_driver_register(&i8042_kbd_driver);
}

DRIVER_INIT(i8042_kbd_init);
