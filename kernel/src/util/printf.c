...
static int print_unsigned(uintmax_t v, int base, bool upper, print_fmt_t *f,
			  bool is_sn, snbuf_t *b)
{
	char buf[65];
	int i = 0;
...
static void parse_len(fmt_ctx_t *c)
{
	if (*c->fmt == 'h') {
		c->fmt++;
		c->len = (*c->fmt == 'h') ? (++c->fmt, PRINT_LEN_HH) :
					    PRINT_LEN_H;
	} else if (*c->fmt == 'l') {
		c->fmt++;
		c->len = (*c->fmt == 'l') ? (++c->fmt, PRINT_LEN_LL) :
					    PRINT_LEN_L;
	} else if (*c->fmt == 'z') {
		c->len = PRINT_LEN_Z;
		c->fmt++;
