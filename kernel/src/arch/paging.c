...
int map_page(ptable_t *pml4, uint64_t vaddr, uint64_t paddr, uint64_t flags)
{
	...
	if (!(entry & PTE_PRESENT)) {
		...
		if (!phys) {
			klog("paging",
			     COL_AMBER "failed to allocate pdpt" COL_RESET);
			return -1;
		}
	...
	if (!(entry & PTE_PRESENT)) {
		...
		if (!phys) {
			klog("paging",
			     COL_AMBER "failed to allocate pd" COL_RESET);
			return -1;
		}
	...
	if (!(entry & PTE_PRESENT)) {
		...
		if (!phys) {
			klog("paging",
			     COL_AMBER "failed to allocate pt" COL_RESET);
			return -1;
		}
	...
	if (entry & PTE_PRESENT) {
		if ((entry & PTE_ADDR_MASK) != (paddr & PTE_ADDR_MASK))
			klog("paging",
			     COL_AMBER
			     "remapping VA=%p (old PA=%p new PA=%p)" COL_RESET,
			     vaddr, entry & PTE_ADDR_MASK, paddr);
	}

	pt->entries[PT_INDEX(vaddr)] = (paddr & PTE_ADDR_MASK) | flags |
				       PTE_PRESENT;
	return 0;
}
...
int unmap_page(ptable_t *pml4, uint64_t vaddr)
{
	...
	if (!(entry & PTE_PRESENT)) {
		klog("paging",
		     COL_AMBER "unmap: missing pml4 entry VA=%p" COL_RESET,
		     vaddr);
		return -1;
	}
	...
	if (!(entry & PTE_PRESENT)) {
		klog("paging",
		     COL_AMBER "unmap: missing pdpt entry VA=%p" COL_RESET,
		     vaddr);
		return -1;
	}
	...
	if (!(entry & PTE_PRESENT)) {
		klog("paging",
		     COL_AMBER "unmap: missing pd entry VA=%p" COL_RESET,
		     vaddr);
		return -1;
	}
	...
	if (!(entry & PTE_PRESENT)) {
		klog("paging",
		     COL_AMBER "unmap: page not mapped VA=%p" COL_RESET, vaddr);
		return -1;
	}
...
#define MAP_SECTION(vstart, vend, flags)                                 \
	do {                                                             \
		...
		for (; _v < _e; _v += PAGE_SIZE) {                       \
			...
			map_page(PHYS_TO_VIRT(kernel_ptable), _v, _p,    \
				 (flags));                               \
		}                                                        \
	} while (0)
	...
	klog("paging", "  limine_requests: 0x%llx -> 0x%llx [RO NX]",
