...
static void device_add_chrdev_node(device_t *dev)
{
	if (char_dev_valid(&dev->chardev))
		devfs_register(dev->name, &dev->chardev);
}

static int device_probe(device_t *dev, driver_t *drv)
{
	if (!dev || !drv || dev->driver)
		return -EINVAL;
	if (!dev->bus || dev->bus != drv->bus || !dev->bus->match(dev, drv))
		return -ENODEV;

	klog("bus", "probing '%s' with '%s'", dev->name, drv->name);
	if (drv->probe) {
