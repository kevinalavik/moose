...
handle_t device_handle_make(void *dev, const dev_ops_t *ops, const char *label)
{
    ...
    return (handle_t){.dev = dev, .ops = ops, .label = label, .type = DEV_TYPE_CHAR};
}
...
size_t device_read(handle_t *h, void *buf, size_t len)
{
    ...
    if (!h->ops->read)
    {
        klog("dev", DEV_COL_AMBER "read unsupported on %s" DEV_COL_RESET, h->label ? h->label : "unknown");
        return 0;
    }
...
size_t device_write(handle_t *h, const void *buf, size_t len)
{
    ...
    if (!h->ops->write)
    {
        klog("dev", DEV_COL_AMBER "write unsupported on %s" DEV_COL_RESET, h->label ? h->label : "unknown");
        return 0;
    }
