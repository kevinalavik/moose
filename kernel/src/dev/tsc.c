...
static uint64_t quick_pit_calibrate(void)
{
    ...
    for (i = 1; i <= MAX_QUICK_PIT_ITERATIONS; i++)
    {
        ...
        delta *= PIT_TICK_RATE;
        delta /= i * 256 * 1000;
        klog("tsc", "fast calibration (ch2): %llu kHz (%llu MHz)", delta, delta / 1000);
        return delta;
    }
...
static uint64_t pit_calibrate_tsc(uint16_t latch, uint64_t ms, int loopmin)
{
    ...
    if (pitcnt < loopmin || tscmin > UINT64_MAX / 10 || tscmax > 10 * tscmin)
        return UINT64_MAX;
...
static uint64_t pit_ch0_calibrate_tsc(uint64_t ms)
{
    ...
    klog("tsc", "ch0 fallback calibration: %llu kHz (%llu MHz)", khz, khz / 1000);
    return khz;
}
...
static uint64_t tsc_do_calibrate(void)
{
    ...
    {
        klog("tsc", "slow calibration (ch2): %llu kHz (%llu MHz)", best, best / 1000);
        return best;
    }
