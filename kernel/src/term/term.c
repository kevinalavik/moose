...
{
	if (n < 16)
		return rgb(t, ansi16_rgb[n].r, ansi16_rgb[n].g,
			   ansi16_rgb[n].b);
	if (n < 232) {
		uint32_t v = n - 16;
		return rgb(t, cube_value(v / 36), cube_value((v / 6) % 6),
			   cube_value(v % 6));
	}
	if (n < 256) {
...
static uint32_t cw(term_t *t)
{
	return t->font.width;
}
static uint32_t ch(term_t *t)
{
...
static void clear_rect(term_t *t, uint32_t x0, uint32_t y0, uint32_t w,
		       uint32_t h, uint32_t color)
{
	...
	for (uint32_t y = 0; y < h; y++) {
		volatile uint32_t *row =
			(volatile uint32_t *)(fb + (y0 + y) * pt + x0 * bpb);
		for (uint32_t x = 0; x < w; x++)
			row[x] = color;
	}
}

static void clear_cells(term_t *t, uint32_t r0, uint32_t c0, uint32_t nr,
			uint32_t nc)
{
...
static uint32_t col_(term_t *t)
{
	return cw(t) == 0 ? 0 : t->cx / cw(t);
}
static uint32_t row_(term_t *t)
{
	return ch(t) == 0 ? 0 : t->cy / ch(t);
}
...
static void clear_line(term_t *t, int mode)
{
	...
	switch (mode) {
	case 0:
		clear_rect(t, t->cx, t->cy, t->fb->width - t->cx, ch_,
			   bg_col(t));
		...
		if ((p == 38 || p == 48) && i + 1 < t->ansi.param_count) {
			...
			if (mode == 2 && i + 4 < t->ansi.param_count) {
				...
				if (r >= 0 && r <= 255 && g >= 0 && g <= 255 &&
				    b >= 0 && b <= 255) {
					if (p == 38)
						set_fg_rgb(t, (uint32_t)r,
							   (uint32_t)g,
							   (uint32_t)b);
					else
						set_bg_rgb(t, (uint32_t)r,
							   (uint32_t)g,
							   (uint32_t)b);
				}
				i += 4;
	       ...
	       size_t psf_size)
{
	...
	reset_attrs(t);
	if (t->fb && t->fb->address)
		clear_rect(t, 0, 0, t->fb->width, t->fb->height,
			   ansi_color(t, TERM_DEFAULT_BG));
	cursor_show(t);
}
