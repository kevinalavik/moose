...
void pmm_init(void)
{
	...
	if (map_phys == (uint64_t)-1) {
		klog("pmm",
		     COL_BRED
		     "could not place page database (need %s)" COL_RESET,
		     size_to_str(map_pages * PAGE_SIZE));
		return;
	}
	...
	klog("pmm", "page database @ %p (%s)", mem_map,
	     size_to_str(map_pages * PAGE_SIZE));
	...
	for (uint64_t i = 0; i < moose_memmap->entry_count; i++) {
		...
		if (end <= start)
			continue;
		klog("pmm", "  0x%016llx - 0x%016llx [%s]", start, end - 1,
		     size_to_str(end - start));
	}
	...
	klog("pmm", "%s free, %s reserved", size_to_str(free_pages * PAGE_SIZE),
...
void pmm_ref(void *ptr)
{
	...
	if (p == NULL) {
		klog("pmm",
		     COL_AMBER
		     "pmm_ref on out-of-range page @ %p, ignored" COL_RESET,
		     ptr);
		return;
	}
	...
	if (!(p->flags & PAGE_USED)) {
		klog("pmm",
		     COL_AMBER
		     "pmm_ref on non-used page @ %p, ignored" COL_RESET,
		     ptr);
		return;
	}
...
void pmm_unref(void *ptr)
{
	...
	if (p == NULL) {
		klog("pmm",
		     COL_AMBER
		     "pmm_unref on out-of-range page @ %p, ignored" COL_RESET,
		     ptr);
		return;
	}
	...
	if (!(p->flags & PAGE_USED)) {
		klog("pmm",
		     COL_AMBER
		     "pmm_unref on non-used page @ %p, ignored" COL_RESET,
		     ptr);
		return;
	}
	...
	if (p->refcount == 0) {
		klog("pmm",
		     COL_AMBER
		     "pmm_unref on zero-refcount page @ %p, ignored" COL_RESET,
		     ptr);
		return;
	}
...
void pmm_free(void *ptr)
{
	...
	if (IS_NOT_ALIGNED((uintptr_t)ptr, PAGE_SIZE)) {
		klog("pmm",
		     COL_AMBER
		     "tried to free unaligned page @ %p, ignored" COL_RESET,
		     ptr);
		return;
	}
	...
	if (p == NULL) {
		klog("pmm",
		     COL_AMBER
		     "tried to free outside PMM range @ %p, ignored" COL_RESET,
		     ptr);
		return;
	}
	...
	if (p->flags & PAGE_RESERVED) {
		klog("pmm",
		     COL_AMBER
		     "tried to free reserved page @ %p, ignored" COL_RESET,
		     ptr);
		return;
	}
	...
	if (!(p->flags & PAGE_USED)) {
		klog("pmm",
		     COL_AMBER
		     "tried to free non-used page @ %p, ignored" COL_RESET,
		     ptr);
		return;
	}
	...
	if (p->refcount > 0) {
		klog("pmm",
		     COL_AMBER
		     "tried to free referenced page @ %p (refcount=%u), ignored" COL_RESET,
		     ptr, p->refcount);
		return;
	}
