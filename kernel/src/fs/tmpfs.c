...
static int tmpfs_create(inode_t *dir, const char *name, mode_t mode,
			inode_t **out);
static int tmpfs_mknod(inode_t *dir, const char *name, mode_t mode, dev_t rdev,
		       inode_t **out);
...
static int tmpfs_mkdir(inode_t *dir, const char *name, mode_t mode)
{
	...
	inode_t *child =
		tmpfs_alloc_inode(dir->i_sb, S_IFDIR | (mode & ~S_IFMT));
	if (!child)
		return -1;
			...
			inode_t **out)
{
	...
	inode_t *child =
		tmpfs_alloc_inode(dir->i_sb, S_IFREG | (mode & ~S_IFMT));
	if (!child)
		return -1;
...
static int tmpfs_readdir(file_t *file, dirent_t *dirent, loff_t *pos)
{
	...
	loff_t i = 0;
	for (struct tmpfs_dirent *e = inode->i_private; e;
	     e = e->td_next, i++) {
		if (i == *pos) {
