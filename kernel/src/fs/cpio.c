...
static int cpio_do_file(inode_t *root, const char *path, const void *data,
...
int cpio_archive_extract(inode_t *root, void *buf, size_t len)
{
	...
	while (p + sizeof(cpio_header_t) <= end) {
		...
		void *data =
			cpio_align((uint8_t *)hdr + sizeof(*hdr) + namesize);
		...
		if (S_ISDIR(mode)) {
			...
			if (err)
				klog("cpio", COL_AMBER "mkdir %s" COL_RESET,
				     name);
			else
				nr_dir++;
			...
			if (err)
				klog("cpio", COL_AMBER "file %s" COL_RESET,
				     name);
			else
				nr_file++;
		} else {
	...
	klog("cpio", "extracted %zu dirs, %zu files (%zu skipped)", nr_dir,
	     nr_file, nr_skip);
	return 0;
}
