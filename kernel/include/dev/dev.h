...
typedef enum { DEV_STATUS_OK = 0, DEV_STATUS_ERROR, DEV_STATUS_INVALID, DEV_STATUS_UNSUPPORTED, DEV_STATUS_NO_DEVICE } dev_status_t;
...
typedef struct dev_ops { dev_read_fn read; dev_write_fn write; } dev_ops_t;
...
#define HANDLE_INVALID ((handle_t){.dev=NULL,.ops=NULL,.label=NULL,.type=DEV_TYPE_NONE})
